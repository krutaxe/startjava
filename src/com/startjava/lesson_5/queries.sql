\echo Вся таблица
select * from robots_and_jaegers
order by model_name;
...
select * from robots_and_jaegers
where launch = (select min(launch) from robots_and_jaegers);

\echo Информация из столбцов model_name, mark, launch, kaiju_kill тех роботов, которые уничтожили больше всех kaiju
select model_name, mark, launch, kaiju_kill from robots_and_jaegers
where kaiju_kill = (select max(kaiju_kill) from robots_and_jaegers);

\echo Средний вес роботов, округлив его до трех знаков после запятой
select round(avg(weight), 3) from robots_and_jaegers;
